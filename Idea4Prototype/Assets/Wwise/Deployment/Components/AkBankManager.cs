...
public static class AkBankManager
{
	public class BankHandle
	{
		...
		public BankHandle(string name, bool decode, bool save)
		{
			bankName = name;
			bankCallback = null;
			decodeBank = decode;
			...
			if (decodeBank)
			{
				...
				string EncodedBankPath = System.IO.Path.Combine(AkBasePathGetter.GetSoundbankBasePath(), bankName + ".bnk");
		...
		public void LoadBankAsync(AkCallbackManager.BankCallback callback = null)
		{
			if (m_RefCount == 0)
			{
				...
				AkSoundEngine.LoadBank(bankName, GlobalBankCallback, this, AkSoundEngine.AK_DEFAULT_POOL_ID, out m_BankID);
			}
			IncRef();
		}
	...
	static public List<BankHandle> BanksToUnload = new List<BankHandle>();
    ...
    {
		m_Mutex.WaitOne();	
		BankHandle handle = (BankHandle)in_Cookie;
    ...
    public static void LoadBank(string name, bool decodeBank, bool saveDecodedBank)
    {
		...
		{
			handle = new BankHandle(name, decodeBank, saveDecodedBank);
	...
	public static void LoadBankAsync(string name, AkCallbackManager.BankCallback callback = null)
	{
		...
		if (!m_BankHandles.TryGetValue(name, out handle))
		{
			handle = new BankHandle(name, false, false);
			m_BankHandles.Add(name, handle);			
			m_Mutex.ReleaseMutex();
